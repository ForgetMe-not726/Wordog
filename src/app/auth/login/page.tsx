  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-lg shadow-orange-100/50 p-8 w-full max-w-sm space-y-4">
      ...
      {params.get("registered") && (
        <p className="bg-orange-50 text-orange-600 text-sm text-center rounded-xl py-2">注册成功，请登录</p>
      )}
      {error && (
      ...
      <p className="text-center text-sm text-gray-400">
        还没有账号？{" "}
        <Link href="/auth/register" className="text-orange-500 font-bold">注册</Link>
      </p>
    </form>
  );
}
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-orange-50 via-orange-50 to-amber-50/30 px-4">
      <Suspense fallback={<div className="w-8 h-8 border-3 border-orange-300 border-t-orange-500 rounded-full animate-spin" />}>
        <LoginForm />
      </Suspense>
    </div>
  );
